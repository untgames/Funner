...
const char* RFX_FILE_NAME             = "media/render.rfx";     //имя файла с настройками рендера
const char* TEXTURE_FILE_NAME         = "media/bottom.jpg";     //имя файла с текстурой
  ...
  : impl (new Impl)
{
  try
  {
    ...
    impl->input_source.RegisterHandler (xtl::bind (&Impl::ProcessInputEvent, impl.get (), _1));
...
{
  impl->render_client.LoadResource (RFX_FILE_NAME);
  impl->render_client.LoadResource (TEXTURE_FILE_NAME);
