...
int main ()
{
  ...
  {
    aes_ofb (TESTS [i].key, TESTS [i].key_size * 8, TESTS [i].data_size, TESTS [i].source, buffer, TESTS [i].iv);

    printf ("Crypto %d correct - %c\n", i, memcmp (buffer, TESTS [i].result, TESTS [i].data_size) ? 'n' : 'y');

    aes_ofb (TESTS [i].key, TESTS [i].key_size * 8, TESTS [i].data_size, buffer, decrypt_buffer, TESTS [i].iv);
