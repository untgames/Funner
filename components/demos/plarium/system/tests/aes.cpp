...
struct Test
{
  ...
  const unsigned char source [BLOCK_SIZE];
  const unsigned char result [BLOCK_SIZE];
};
...
Test TESTS [] = {
 ...
 { 32,
   ...
   { 0xb2, 0xe2, 0x1f, 0xdf, 0x7a, 0xa6, 0x2a, 0x8b, 0xdb, 0x54, 0x37, 0x43, 0x75, 0xc5, 0x8a, 0xb4 },
   { 0x41, 0x15, 0xe3, 0xc6, 0x45, 0xc1, 0x9f, 0x02, 0xf5, 0x43, 0xac, 0x9e, 0x03, 0x92, 0x3c, 0xb6 }
 }
};
...
int main ()
{
  ...
  unsigned char buffer [BLOCK_SIZE], decrypt_buffer [BLOCK_SIZE];

  for (size_t i = 0, count = sizeof (TESTS) / sizeof (*TESTS); i < count; i++)
  {
    aes (CryptoOperation_Encrypt, TESTS [i].key, TESTS [i].key_size * 8, 1, TESTS [i].source, buffer);

    printf ("Crypto %d correct - %c\n", i, memcmp (buffer, TESTS [i].result, BLOCK_SIZE) ? 'n' : 'y');

    aes (CryptoOperation_Decrypt, TESTS [i].key, TESTS [i].key_size * 8, 1, buffer, decrypt_buffer);

    printf ("Decrypt correct - %c\n", memcmp (TESTS [i].source, decrypt_buffer, BLOCK_SIZE) ? 'n' : 'y');
  }
  ...
  for (size_t i = 0, count = sizeof (TESTS) / sizeof (*TESTS); i < count; i++)
  {
    ...
    context.Update (1, TESTS [i].source, buffer);

    printf ("Crypto %d correct - %c\n", i, memcmp (buffer, TESTS [i].result, BLOCK_SIZE) ? 'n' : 'y');

    AesContext decrypt_context (CryptoOperation_Decrypt, TESTS [i].key, TESTS [i].key_size * 8);

    decrypt_context.Update (1, buffer, decrypt_buffer);
