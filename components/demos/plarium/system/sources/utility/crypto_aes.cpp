...
const size_t CRYPTO_BLOCK_SIZE = 16;
...
void aes_init (AesParam* ap, const byte* key, size_t key_bits, CryptoOperation operation)
{
  ...
  if (operation == CryptoOperation_Decrypt)
  {
    rk = ap->key_expansion;
...
struct AesContext::Impl
{
  ...
  Impl (CryptoOperation operation, const void* key, size_t key_bits)
  {
    ...
    switch (operation)
    {
      case CryptoOperation_Encrypt:
        cc_operation = kCCEncrypt;
        break;
  ...
  void Update (size_t blocks_count, const void* src_buffer, void* dst_buffer)
  {
    ...
    CheckCCError ("AesContext::Update", CCCryptorUpdate (cryptor, src_buffer, buffer_size, dst_buffer, buffer_size, &encoded_bytes));
  }

  void CheckCCError (const char* source, CCCryptorStatus status)
  {
    switch (status)
    {
      case kCCSuccess:
...
struct AesContext::Impl
{
  ...
  Impl (CryptoOperation in_operation, const void* key, size_t key_bits)
  ...
  void Update (size_t blocks_count, const void* src_buffer, void* dst_buffer)
  {
    ...
    for (; blocks_count; blocks_count--, src += CRYPTO_BLOCK_SIZE, dst += CRYPTO_BLOCK_SIZE)
    {
      switch (operation)
      {
        case CryptoOperation_Encrypt:
          aes_encrypt (&context, (aes_uint32_t*)dst, (const aes_uint32_t*)src);
          break;
        case CryptoOperation_Decrypt:
          aes_decrypt (&context, (aes_uint32_t*)dst, (const aes_uint32_t*)src);
          break;
      }
    }
  }
...
AesContext::AesContext (CryptoOperation operation, const void* key, size_t key_bits)
  : impl (new Impl (operation, key, key_bits))
  {}
...
void AesContext::Update (size_t blocks_count, const void* src_buffer, void* dst_buffer)
{
  impl->Update (blocks_count, src_buffer, dst_buffer);
}
