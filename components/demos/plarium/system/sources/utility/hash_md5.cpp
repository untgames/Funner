...
struct Md5Context::Impl
{
  ...
  {
    if (!data && data_size)
      throw sgi_stl::invalid_argument ("Md5Context::Update - null data");

    if (need_init)
      Init ();

    CC_MD5_Update (&context, data, data_size);
  }

  void Finish (unsigned char (&result_hash_value) [16])
  {
    if (!result_hash_value)
      throw sgi_stl::invalid_argument ("Md5Context::Finish - null result_hash_value");

    if (need_init)
      Init ();

    CC_MD5_Final (result_hash_value, &context);
...
void Md5Final (unsigned char result_hash_value[16], MD5_CTX *context)
{
  ...
  Encode (result_hash_value, context->state, 16);
}
...
void CC_MD5 (const void* data, size_t size, unsigned char result_hash_value [16])
{
  MD5_CTX context;
...
struct Md5Context::Impl
{
  ...
  void Update (const void* data, size_t data_size)
  {
    ...
    Md5Update (&context, (unsigned char*)data, data_size);
  }
...
{
  if (!data && size)
    throw sgi_stl::invalid_argument ("plarium::utility::md5 - null data");

  CC_MD5 (data, size, result_hash_value);
}
...
void Md5Context::Update (const void* data, size_t data_size)
{
