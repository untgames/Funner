...
/*
    »нтерпретатор, которому принадлежит стек
*/
...
xtl::any& Stack::GetVariant (size_t index)
{
  ...
  {
      //все пользовательские типы данных, хранимые в стеке, привод€тс€ к xtl::any*. проверка совпадени€ метатаблиц не требуетс€
...
void Stack::Push (const xtl::any& value)
{
  ...
  if (value.null ())
  {
      //обработка специального случа€: помещение в стек нулевого указател€
