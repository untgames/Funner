...
///Конструктор
          ...
          log_handler (in_log_handler)
    {
      try
      {
        ...
        draw_transaction_manager = new render::obsolete::DrawTransactionManager (*renderer, log_handler);
...
///Получение менеджера транзакций отрисовки
    render::obsolete::DrawTransactionManager& DrawTransactionManager () { return *draw_transaction_manager; }
...
///Протоколирование
  ...
  private:
    ...
    typedef stl::vector<render::obsolete::RenderTargetImpl*>                RenderTargetList;
    typedef xtl::intrusive_ptr<FrameBuffer>                       FrameBufferPtr;
    typedef stl::vector<FrameBufferPtr>                           FrameBufferArray;
    typedef stl::auto_ptr<render::obsolete::DrawTransactionManager>         DrawTransactionManagerPtr;
