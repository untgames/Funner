...
///Создание цепочки обмена
  void CreateSwapChain (syslib::Window& window)
  {
    ...
    {
      e.touch ("render::mid_level::WindowImpl::Impl::CreateSwapChain");
      throw;
    }
  }
...
///Создание целей рендеринга
  void CreateRenderTargets ()
  {
    ...
    {
      e.touch ("render::mid_level::WindowImpl::Impl::CreateRenderTargets");
      throw;
    }
  }
...
///Обновление размеров окна
  void UpdateSizes (syslib::Window& window)
  {
    ...
    {
      e.touch ("render::mid_level::WindowImpl::Impl::UpdateSizes");
      throw;
    }
  }
...
///Обработка события перерисовки окна
  void OnUpdate ()
  {
    ...
    {
      log.Printf ("%s\n    at render::mid_level::WindowImpl::Impl::OnUpdate", e.what ());      
    }
    catch (...)
    {
      log.Printf ("unknown exception\n    at render::mid_level::WindowImpl::Impl::OnUpdate");
    }
  }
...
///Обработка события изменения размеров окна
  void OnResize (syslib::Window& window)
  {
    ...
    {
      log.Printf ("%s\n    at render::mid_level::WindowImpl::Impl::OnResize", e.what ());
    }
    catch (...)
    {
      log.Printf ("unknown exception\n    at render::mid_level::WindowImpl::Impl::OnResize");
    }
  }
...
///Обработка события смены оконного дескриптора
  void OnChangeHandle (syslib::Window& window)
  {
    ...
    {
      log.Printf ("%s\n    at render::mid_level::WindowImpl::Impl::OnChangeHandle", e.what ());
    }
    catch (...)
    {
      log.Printf ("unknown exception\n    at render::mid_level::WindowImpl::Impl::OnChangeHandle");
    }
  }
};
...
WindowImpl::WindowImpl (const DeviceManagerPtr& device_manager, syslib::Window& window, const common::PropertyMap& properties)
{
  try
  {
    ...
    if (!device_manager)
    {
      ...
      impl->device_manager = DeviceManagerPtr (new mid_level::DeviceManager (device, driver_mask, adapter_mask), false);
    }
    else
    {
  ...
  {
    e.touch ("render::mid_level::WindowImpl::WindowImpl");
    throw;
  }
}
...
{
  if (!name)
    throw xtl::make_null_argument_exception ("render::mid_level::WindowImpl::SetName", "name");
...
const RenderTargetPtr& WindowImpl::ColorBuffer ()
{
  if (!impl->color_buffer)
    throw xtl::format_operation_exception ("render::mid_level::WindowImpl::ColorBuffer", "No color buffer binded");
...
const RenderTargetPtr& WindowImpl::DepthStencilBuffer ()
{
  if (!impl->depth_stencil_buffer)
    throw xtl::format_operation_exception ("render::mid_level::WindowImpl::DepthStencilBuffer", "No depth-stencil buffer binded");
...
xtl::connection WindowImpl::RegisterEventHandler (WindowEvent event, const EventHandler& handler) const
{
  switch (event)
  {
    ...
    default:
      throw xtl::make_argument_exception ("render::mid_level::WindowImpl::RegisterEventHandler", "event", event);
  }
}
