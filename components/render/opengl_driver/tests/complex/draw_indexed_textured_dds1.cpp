...
PixelFormat get_format (const char* image_format)
{
  if (!xtl::xstricmp (image_format, "dxt1"))      return PixelFormat_DXT1;
  else if (!xtl::xstricmp (image_format, "dxt3")) return PixelFormat_DXT3;
  else if (!xtl::xstricmp (image_format, "dxt5")) return PixelFormat_DXT5;
  else
    throw xtl::format_not_supported_exception ("::get_format", "Unsupported compression format '%s'", image_format);
}
