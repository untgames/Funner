  ...
  : Texture (manager, tex_desc, GL_TEXTURE_3D_EXT, get_mips_count (tex_desc.width, tex_desc.height, tex_desc.layers))
{
  ...
  switch (GetFormat ())
  {
    ...
    case PixelFormat_DXT5:
      raise_not_supported (METHOD_NAME, "3D compressed textures not supported (desc.format=%s)", get_name (GetFormat ()));
      return;
    case PixelFormat_D16:
    case PixelFormat_D24X8:
    case PixelFormat_D24S8:
    case PixelFormat_S8:    
      raise_not_supported (METHOD_NAME, "3D depth-stencil textures not supported (desc.format=%s)", get_name (GetFormat ()));
      return;
    default:
      raise_invalid_argument (METHOD_NAME, "desc.format", GetFormat ());
      return;
  }
  ...
  if (!width)
    raise_not_supported (METHOD_NAME, "Can't create 3d texture %ux%ux%u@%s (proxy texture fail)", 
...
/*
    –абота с данными
      данна€ перегрузка обсуловлена необходимостью обхода некорректной работы функции glGetTexImage в драйверах ATI
*/
...
{  
  static const char* METHOD_NAME = "render::low_level::opengl::Texture3D::GetData";
    ...
    //если баг работы функции glGetTexImage не обнаружен - копирование производитс€ как и в общем случае
...
{
  raise_not_supported ("render::low_level::opengl::Texture3D::SetCompressedData", "Compression for 3D textures not supported");
}
