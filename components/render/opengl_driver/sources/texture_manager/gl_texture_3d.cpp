  ...
  : Texture (manager, tex_desc, GL_TEXTURE_3D_EXT, get_mips_count (tex_desc.width, tex_desc.height, tex_desc.layers))
{
  ...
  if (!width)
    ...
    //настройка расположени€ данных в буфере
  ...
  glPixelStorei (GL_PACK_IMAGE_HEIGHT_EXT, 0); //высота сло€ в пиксел€х
  ...
  for (size_t mip_level=0; mip_level<GetMipsCount (); mip_level++)
  {
    ...
    glPixelStorei (GL_UNPACK_ROW_LENGTH, level_desc.width); //длина строки в пиксел€х (дл€ данного mip-уровн€)
...
{  
    /*
        ‘ункци€ всегда проходит через "ручное" копирование, поскольку определить баг на старых верси€х драйверов ATI невозможно
