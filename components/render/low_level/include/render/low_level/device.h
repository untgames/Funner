...
struct DeviceCaps
{
  unsigned int max_texture_1d_size;            //максимальная длина одномерной текстуры
  unsigned int max_texture_2d_size;            //максимальный размер ребра двумерной текстуры
  unsigned int max_texture_3d_size;            //максимальный размер ребра трёхмерной текстуры
  unsigned int max_texture_cubemap_size;       //максимальный размер ребра кубической текстуры
  unsigned int max_texture_anisotropy;         //максимальный уровень анизотропии
  unsigned int samplers_count;                 //количество текстурных сэмплеров
...
///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    virtual IProgram*                 CreateProgram                 (unsigned int shaders_count, const ShaderDesc* shader_descs, const LogFunction& error_log) = 0;
    virtual ITexture*                 CreateTexture                 (const TextureDesc&) = 0;
    virtual ITexture*                 CreateTexture                 (const TextureDesc&, const TextureData&) = 0;
    virtual ITexture*                 CreateTexture                 (const TextureDesc&, IBuffer* buffer, unsigned int buffer_offset, const unsigned int* mip_sizes) = 0;
    virtual ITexture*                 CreateRenderTargetTexture     (ISwapChain* swap_chain, unsigned int buffer_index) = 0;
