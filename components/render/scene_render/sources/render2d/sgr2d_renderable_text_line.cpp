...
RenderableTextLine::RenderableTextLine (scene_graph::TextLine* in_text_line, Render& in_render)
  : Renderable (in_text_line),
    render (in_render),
  ...
  {    
    ...
    if (need_rebuild_text)
    {
      ...
      for (size_t i = 0; i < sprites_buffer.size (); i++, current_sprite++)
      {
        ...
        current_sprite->position = math::vec3f (current_pen_x_position + bearing_x, current_pen_y_position + bearing_y, 0.f);
    ...
    {
      primitive->SetTransform (translate_tm * world_tm);
