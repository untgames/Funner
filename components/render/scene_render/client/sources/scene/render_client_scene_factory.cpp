...
Node* SceneFactory::Create (scene_graph::Node& src_node, SceneManager& scene_manager)
{
  if (scene_graph::Scissor* node = dynamic_cast<scene_graph::Scissor*> (&src_node))
    return new Entity (*node, scene_manager, interchange::NodeType_Scissor); 
  ...
  if (scene_graph::DynamicMesh* node = dynamic_cast<scene_graph::DynamicMesh*> (&src_node))
    return create_node (*node, scene_manager);

  if (scene_graph::ParticleEmitter* node = dynamic_cast<scene_graph::ParticleEmitter*> (&src_node))
    return create_node (*node, scene_manager);
