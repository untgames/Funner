...
size_t GetBpp (PixelFormat pf)
{
  switch (pf)
  {
    ...
    case PixelFormat_RGB16:  return 6;
...
int main (int argc, char *argv[])
{
  if (argc != 2)
  {
    printf ("Usage: image2xfont imagename\n");
    return 1;
  }
  ...
  try
  {
    ...
    font.SetFirstGlyphCode (32);

    mask [0] = 0;

    for (size_t i = 0; i < img.Height (); i++)
      for (size_t j = 0; j < img.Width (); j++)
        if (mask [i * img.Width () + j] && !memcmp (border_color, &border_color[(i * img.Width () + j ) * bpp], bpp))
        {
