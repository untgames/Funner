...
const size_t DEFAULT_EVENTS_TO_FIRE_ARRAY_SIZE = 4;
...
struct EventDesc : public xtl::reference_counter
{
  ...
  EventDesc (float in_delay, float in_period, const char* in_event)
    : delay (in_delay), period (in_period), event (in_event)
    {}
};
...
size_t EventTrack::AddEvent (float delay, float period, const char* event)
{
  ...
  if (delay < 0)
    throw xtl::make_argument_exception (METHOD_NAME, "delay", delay);

  if (period < 0)
    throw xtl::make_argument_exception (METHOD_NAME, "period", period);

  impl->events.push_back (EventDescPtr (new EventDesc (delay, period, event)));
...
void EventTrack::SetDelay (size_t event_index, float delay)
{
  ...
  if (event_index >= impl->events.size ())
    throw xtl::make_range_exception (METHOD_NAME, "event_index", event_index, 0u, impl->events.size ());
...
void EventTrack::SetEvent (size_t event_index, float delay, float period, const char* event)
{
  ...
  impl->events [event_index] = EventDescPtr (new EventDesc (delay, period, event));
}
...
{
  if (event_index >= impl->events.size ())
    throw xtl::make_range_exception ("media::animation::EventTrack::GetEvent", "event_index", event_index, 0u, impl->events.size ());
...
{
  if (event_index >= impl->events.size ())
    throw xtl::make_range_exception ("media::animation::EventTrack::GetTime", "event_index", event_index, 0u, impl->events.size ());
...
void EventTrack::GetEvents (float previous_time, float current_time, const EventHandler& handler) const
{
  ...
  for (EventDescArray::iterator iter = impl->events.begin (), end = impl->events.end (); iter != end; ++iter)
  {
    ...
    float clamped_previous_time = fmod (previous_time - delay, period),
          clamped_current_time  = fmod (current_time - delay, period);

    if (clamped_previous_time < clamped_current_time)
      impl->events_to_fire.push_back (*iter);
  }

  size_t events_to_fire_count = impl->events_to_fire.size ();

  for (float t = previous_time; t < current_time;)
  {
