...
struct Option
{
  ...
  const char*                name;          //им€ команды
  char                       short_name;    //короткое им€
  const char*                argument_name; //им€ аргумента
...
struct Params
{
  ...
  StringArray   sources;                  //им€ исходных изображений
...
void build (Params& params)
{
  try
  {
    ...
    for (size_t i = 0, count = params.sources.Size (); i < count; i++)
    {
      ...
      if (i > 0)  //операци€ не выполн€етс€ дл€ первого кадра в последовательности
      {
        ...
        //определение имени результирующего изображени€
