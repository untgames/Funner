...
media::rfx::Material::ConstPointer MultiPassMaterial::Material (size_t index) const
{
  if (index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::Material", "index", index, impl->passes.size ());
...
{
  if (index > impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::InsertPass", "index", index, impl->passes.size ());

  if (!material_ptr)
    throw xtl::make_null_argument_exception ("media::rfx::MultiPassMaterial::InsertPass", "material_ptr");
...
{
  if (index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::SetMaterial", "index", index, impl->passes.size ());

  if (!material_ptr)
    throw xtl::make_null_argument_exception ("media::rfx::MultiPassMaterial::SetMaterial", "material_ptr");
...
void MultiPassMaterial::MovePass (size_t source_index, size_t destination_index)
{
  if (source_index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::MovePass", "source_index", source_index, impl->passes.size ());

  if (destination_index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::MovePass", "destination_index", destination_index, impl->passes.size ());
...
void MultiPassMaterial::SwapPasses (size_t index1, size_t index2)
{
  if (index1 >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::SwapPasses", "index1", index1, impl->passes.size ());

  if (index2 >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::SwapPasses", "index2", index2, impl->passes.size ());
...
{
  if (!name)
    throw xtl::make_null_argument_exception ("media::rfx::MultiPassMaterial::FindPass", "name");
...
{
  if (index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::PassState", "index", index, impl->passes.size ());
...
{
  if (index >= impl->passes.size ())
    throw xtl::make_range_exception ("media::rfx::MultiPassMaterial::SetPassState", "index", index, impl->passes.size ());
  ...
  switch (state)
  {
    ...
    case PassState_Disabled:
      break;
    default:
      throw xtl::make_argument_exception ("media::rfx::MultiPassMaterial::SetPassState", "state", state);
      break;
  }
