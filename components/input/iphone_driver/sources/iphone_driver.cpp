...
const DeviceDesc SUPPORTED_DEVICES [] = { APPLICATION_DEVICE_DESC, ASCII_KEYBOARD_DEVICE_DESC,
    ...
    IDevice* CreateDevice (const char* name, const char* init_string = "") 
    {
      ...
      for (size_t i = 0, count = GetDevicesCount (); i < count; i++)
      {
        ...
        if (!xtl::xstrcmp (device_name, name))
        {
          ...
          if (!xtl::xstrcmp (device_name, ASCII_AUTOCAPITALIZED_KEYBOARD_DEVICE_DESC.full_name))
          ...
          if (!xtl::xstrcmp (device_name, NUMBER_KEYBOARD_DEVICE_DESC.full_name))
          ...
          if (!xtl::xstrcmp (device_name, NUMBER_PUNCTUATION_KEYBOARD_DEVICE_DESC.full_name))
          ...
          if (!xtl::xstrcmp (device_name, APPLICATION_DEVICE_DESC.full_name))
