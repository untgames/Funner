...
{  
  ...
  if (!iter)
    parser.Log ().Error (*iter, "'UIConfig' tag missing");
  ...
  for (size_t i=0, count=log.MessagesCount (); i<count; i++)
    window_system.LogPrintf ("%s", log.Message (i));
}
...
{
  const char* source = get<const char*> (*menu_iter, "Source", (const char*)0);
  ...
  if (source) //обработка инстанцированных элементов меню
  {
    ...
    if (!instance)
      parser.Log ().Error (*menu_iter, "MenuItem '%s' not found", source);

    return instance;
  }
  
  const char *id       = get<const char*> (*menu_iter, "Id", ""),
             *text     = get<const char*> (*menu_iter, "Text"), //no default value
...
{
  const char* source = get<const char*> (*button_iter, "Source", (const char*)0);
  ...
  if (source) //обработка инстанцированных элементов меню
  {
    ...
    if (!instance)
      parser.Log ().Error (*button_iter, "ToolButton '%s' not found", source);
  ...
  if (!*text && !*image)
  {
    parser.Log ().Error (*button_iter, "'Text' and 'Image' tags missing");
    return 0;
  }
