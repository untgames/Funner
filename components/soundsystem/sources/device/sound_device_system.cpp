...
{
  if (!driver_name)
    RaiseNullArgument ("sound::low_level::SoundSystem::RegisterDriver", "driver_name");
    
  if (!creater)
    RaiseNullArgument ("sound::low_level::SoundSystem::RegisterDriver", "creater");
    
  DriverMap::iterator iter = drivers.find (driver_name);
  
  if (iter != drivers.end ())
    RaiseInvalidArgument ("sound::low_level::SoundSystem::RegisterDriver", "driver_name", driver_name,
...
{
  if (!driver_name)
    RaiseNullArgument ("sound::low_level::SoundSystem::RegisterConfiguration", "driver_name");

  if (!device_name)
    RaiseNullArgument ("sound::low_level::SoundSystem::RegisterConfiguration", "device_name");

  DriverMap::iterator driver_iter = drivers.find (driver_name);

  if (driver_iter == drivers.end ())
    RaiseInvalidArgument ("sound::low_level::SoundSystem::RegisterConfiguration", "driver_name", driver_name,
...
ISoundDevice* SoundSystemImpl::CreateDevice (const char* configuration_name, const char* init_string)
{
  ...
  if (cfg_iter == configurations.end ())
    RaiseInvalidArgument ("sound::low_level::SoundSystem::CreateDevice", "configuration_name", configuration_name,
