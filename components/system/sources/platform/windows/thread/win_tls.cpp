...
///Очистка для текущей нити
  ...
  private:
    ...
    typedef stl::hash_multimap<DWORD, tls_t> CleanupMap;
...
tls_t WindowsThreadManager::CreateTls (IThreadCleanupCallback* cleanup)
{
  try
  {
  ...
  {
    exception.touch ("syslib::WindowsThreadManager::CreateTls");
    throw;
  }
}

void WindowsThreadManager::DestroyTls (tls_t tls)
{
  if (!tls)
    return;
...
void WindowsThreadManager::SetTls (tls_t tls, void* data)
{
  try
  {
    if (!tls)
  ...
  {
    exception.touch ("syslib::WindowsThreadManager::SetTls");
    throw;
  }
}

void* WindowsThreadManager::GetTls (tls_t tls)
{
  try
  {
    if (!tls)
  ...
  {
    exception.touch ("syslib::WindowsThreadManager::GetTls");
    throw;
  }
}
