...
///Начало получения событий от сенсора
    ...
    {            
      ...
      jboolean status = get_env ().CallBooleanMethod (GetSensorManager ().get (), register_listener_method, handle.event_listener.get (), handle.sensor.get (), SENSOR_DELAY_NORMAL);
