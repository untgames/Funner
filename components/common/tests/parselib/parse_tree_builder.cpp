...
{
  printf ("Found %u nodes with name '%s' (NextNamesake test)\n", get_nodes_count1 (node, name), name);
  printf ("Found %u nodes with name '%s' (Next(const char*) test)\n", get_nodes_count2 (node, name), name);  
}
...
int main ()
{
  ...
  try
  {
    ...
    printf ("builder-copy results (nodes=%u, attributes=%u, buffer_size=%u):\n", builder_copy.TotalNodesCount (),
    ...
    printf ("builder results (nodes=%u, attributes=%u, builder_size=%u):\n", builder.TotalNodesCount (),
      builder.TotalAttributesCount (), builder.BufferSize ());
      
    swap (builder.Build (), root);
