...
class Test
{
  public:
    ...
    template <class T> void TranslateArray (const char* type_name, const char* tag, size_t count);
...
{
  T object = get<T> (parser.Root (), tag, default_value);
...
int main ()
{
  ...
  try
  {
    ...
    test.TranslateArray<float>  ("float", "mat3f_value", 9);
  }
