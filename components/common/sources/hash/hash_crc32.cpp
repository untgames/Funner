...
inline size_t crc32 (unsigned char data,size_t crc)
{
  ...
  return crc_tableil8_o32[(crc ^ data) & 0xff] ^ (crc >> 8);
}
...
size_t crc32 (const void* data, size_t data_size, size_t crc)
{
  ...
  for (size_t li = 0; li < initial_bytes; li++)
    crc = crc_tableil8_o32[(crc ^ *p_buf++) & 0xff] ^ (crc >> 8);
  ...
  else
  {
    ...
    for (size_t li = 0; li < running_length / 8; li++)
    {
        ...
        crc = crc ^ term1 ^ crc_tableil8_o40[term2  & 0xff] ^ crc_tableil8_o32[(term2 >> 8) & 0xff];
        p_buf += 4;
    }
