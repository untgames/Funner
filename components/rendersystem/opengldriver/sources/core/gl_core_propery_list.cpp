...
const char* PropertyList::GetKey (size_t index)
{
  if (index >= impl->properties.size ())
    RaiseOutOfRange ("render::low_level::opengl::PropertyList::GetKey", "index", index, impl->properties.size ());
...
{
  if (index >= impl->properties.size ())
    RaiseOutOfRange ("render::low_level::opengl::PropertyList::GetValue", "index", index, impl->properties.size ());
...
{
  if (!key)
    RaiseNullArgument ("render::low_level::opengl::PropertyList::AddProperty", "key");

  if (!value)
    RaiseNullArgument ("render::low_level::opengl::PropertyList::AddProperty", "value");
