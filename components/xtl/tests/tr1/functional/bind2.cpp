...
void function_object_test()
{
    ...
    TEST ( bind<short>(Y(), ref(i))() == 7 );
    TEST ( bind<short>(Y(), ref(i))() == 8 );
    TEST ( bind<int>(Y(), i, _1)(k) == 38 );
    TEST ( bind<long>(Y(), i, _1, 9)(k) == 938 );

#if !defined(__MWERKS__) || (__MWERKS__ > 0x2407)     // Fails for this version of the compiler.

    global_result = 0;
    bind<void>(Y(), i, _1, 9, 4)(k);
    TEST ( global_result == 4938 );
