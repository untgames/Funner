...
{
	psd_short					channel_id;		// 2 bytes for Channel ID: 0 = red, 1 = green, etc.; ЁC1 = transparency mask; ЁC2 = user supplied layer mask
...
struct _psd_layer_record
{
	...
	psd_bool					clipping;			// 0 = base, 1 = nonЁCbase
...
// HUE/SATURATION
// Hue/Saturation settings files are loaded and saved in PhotoshopЎЇs Hue/Saturation dialog
typedef struct _psd_layer_hue_saturation
{
	...
	psd_short					colorization_saturation;// Photoshop 4.0: Three psd_short integers Hue, Saturation, and Lightness from ЁC100...100.
	psd_short					colorization_lightness;	// The user interface represents hue as ЁC180...180, saturation as 0...100, and Lightness as -100...1000, as the traditional HSB color wheel, with red = 0.
	psd_short					master_hue;				// Master hue, saturation and lightness values.
	psd_short					master_saturation;
	psd_short					master_lightness;
	psd_short					range_values[6][4];		// For RGB and CMYK, those values apply to each of the six hextants in the HSB color wheel: those image pixels nearest to red, yellow, green, cyan, blue, or magenta. These numbers appear in the user interface from ЁC60...60, however the slider will reflect each of the possible 201 values from ЁC100...100.
	psd_short					setting_values[6][3];	// For Lab, the first four of the six values are applied to image pixels in the four Lab color quadrants, yellow, green, blue, and magenta. The other two values are ignored ( = 0). The values appear in the user interface from ЁC90 to 90.
	psd_uchar					lookup_table[6][360];
} psd_layer_hue_saturation;


// SELECTIVE  COLOR
// Selective Color settings files are loaded and saved in PhotoshopЎЇs Selective Color dialog.
typedef struct _psd_layer_selective_color
{
	...
	psd_short					cyan_correction[10];	// Amount of cyan correction. Short integer from ЁC100...100.
	psd_short					magenta_correction[10];	// Amount of magenta correction. Short integer from ЁC100...100.
	psd_short					yellow_correction[10];	// Amount of yellow correction. Short integer from ЁC100...100.
	psd_short					black_correction[10]; 	// Amount of black correction. Short integer from ЁC100...100.
