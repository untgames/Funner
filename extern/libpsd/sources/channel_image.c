...
psd_status psd_get_layer_channel_image_data(psd_context * context, psd_layer_record * layer)
{
	...
	for(i = 0; i < layer->number_of_channels; i ++)
	{
		...
		switch(compression)
		{
			...
			// whose size is calculated as (LayerBottomЁCLayerTop)*
			// (LayerRightЁCLayerLeft).
			case 0:
				if(layer->channel_info[i].channel_id == -2)
			...
			// for all the scan lines in the channel (LayerBottomЁCLayerTop), with
			// each count stored as a twoЁCbyte value.(**PSB** each count stored as a
