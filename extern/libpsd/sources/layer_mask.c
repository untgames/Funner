...
{
	// Type. 4 possible values, 0 = any other type of layer, 1 = open Ў°folderЎ±, 2 =
	// closed Ў°folderЎ±, 3 = bounding section divider, hidden in the UI
...
static psd_status psd_get_layer_info(psd_context * context)
{
	...
	for(i = 0, layer = context->layer_records; i < context->layer_count; i ++, layer ++)
	{
		...
		// ЁC1 = transparency mask; ЁC2 = user supplied layer mask
...
static psd_status psd_get_mask_info(psd_context * context)
{
	...
	// Kind. 0 = Color selectedЎЄi.e. inverted; 1 = Color protected;128 = use
